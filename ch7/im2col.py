...
def im2col(input_data, filter_h, filter_w, stride = 1, pad = 0):
    ...
    for y in range(filter_h):
        ...
        for x in range(filter_w):
            ...
            # y:y_max:stride的长度为(y_max-y)/stride,也就等于out_h;
            # x:x_max:stride的长度为(x_max-x)/stride，等于out_w

            #col6维数组，固定3、4维，剩余维与img的四维对应，把img[:, :, y:y_max:stride,x:x_max:stride]赋值给col的另外四维
            #N-N
            # C-C
            # out_h  --  y:y_max:stride
            # out_w  --  x:x_max:stride
